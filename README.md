**DISCLAIMER:** _IUSB is not officially affiliated to the International University 
of Applied Sciences (IU), Germany._

# International University of Applied Sciences Students Bank (IUSB)
...
Open PowerShell in lambda folder (or the project fold use "cd lambda"):
Then run:
...
#### Outcome:
"npm install" will generate "node_modules" directory and "package-lock.json" file in the
"lambda" directory.

The following code generates a lambda_function.zip in the root folder to assist Lambda deployment.
    
    Compress-Archive -Path * -DestinationPath ../lambda_function.zip

#### Outcome:
"The code will generate "lambda_function.zip" in the main directory (IUCloudProject_IUBS).
...
#### Purpose:
The S3 bucket exist to hold the website's files.

**Deployment Guide:**
● In the main.tf: Block 1 is to be uncommented and keep Blocks 2 to 8 commented-out.
● In the outputs.tf: Block 1 is to be uncommented and keep Blocks 2 and 3 commented-out.
...
#### Purpose:
These block deploy the IUSB website's HTML and CSS files.
The static HTML and CSS files will be in S3 first, however, API and Lambda is not ready.

**Deployment Guide:**
● In the main.tf:
    Block 2a is to be uncommented now while keeping Block 1 also uncommented.
    keep Blocks 2b to 8 commented-out.
    Wait until API is deployed before uncommenting Block 2b.
● In the outputs.tf: Block 1 is to be kept uncommented while keeping Blocks 2 and 3 commented-out.
● Then run the following code one after the other:
...
#### Outcome:
Now HTML and CSS files are uploaded. Test by opening the S3 website endpoint; index.html should load.

### STEP 3 — Deploy DynamoDB Tables: Block 3

#### Purpose:
DynamoDB is required for backend storage.
Blocks 3 is the users table and Block 4 is the counters table.
...
#### Outcome:
DynamoDB tables users and counters are created. Check DynamoDB console.

### STEP 4 — Create IAM Role for Lambda: Block 5

#### Purpose:
Lambda needs IAM role (plus policy) with DynamoDB and CloudWatch permissions.
...
● In the outputs.tf: Keep Block 1 and Blocks 2 uncommented, but Block 2 commented-out.
...
#### Outcome:
IAM role is created. Check AWS IAM console.
...
#### Outcome:
Lambda is deployed. Check AWS Lambda console.
...
● Deployment Guide (actions to be taken in the main.tf):  
    Uncomment block 7 (API Gateway and Routes)
    Keep blocks 1 to 6 uncommented, except Block 2b
    Keep Block 8 commented-out.
● Deployment Guide (in the outputs.tf): Uncomment API outputs (Block 2) in outputs.tf.
...
#### Outcome:
API Gateway deployed. Terraform output will show api_url.

### STEP 7 — Generate app.js with API URL: Block 8

#### Purpose:
Inject real API URL (local_file app.js) into JS for frontend.
...
#### Outcome:
website app.js is generated with API URL.
...
#### Outcome:
IUSB website is now fully functional such that
1. HTML and CSS are from S3 Bucket.
2. JS calls Lambda via API Gateway.
3. Registration and visitor counter should work now.
...
**Note:** JS automatically calls API Gateway endpoints. Do not access API Gateway root URL directly.



**DISCLAIMER:** _IUSB is not officially affiliated to the International University
of Applied Sciences (IU), Germany_
